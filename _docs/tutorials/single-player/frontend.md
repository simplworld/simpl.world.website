...
For the `game_slug` value, enter `calc` (the slug value you used in the [Single-player Game Model Service Tutorial]({% link _docs/tutorials/single-player/modelservice.md %})). 
...
The user's specific information will be fetched from the [Simpl Games API]({% link _docs/getting-started.md %}) and kept in sync automatically.
